...
export function farmingSearch({ city, zip, lat, lng, radius = 1.0, signals = [], limit = 50, minScore = 0, minAcres = 0, minValue = 0 }) {
  ...
  const db = getClerkDb();
  // Keep only lightweight {score, p, reasons, clerkHits} per candidate while
  // scanning — the heavy prospect object (nested property/mailing/sales/court +
  // bridge URL) is built ONLY for the returned top-`limit` below. Materializing
  // all ~8000 candidates as heavy objects just to slice to a handful ballooned
  // V8's heap high-water mark to ~1GB/request under load, which on this
  // swap-pressured box caused event-loop stalls (the 2026-08-02 total timeout).
  const scored = [];
  ...
  for (const p of records) {
    ...
    if (score >= 70) highN++; else if (score >= 40) medN++; else lowN++;
    scored.push({ score, p, reasons, clerkHits });
  }

  scored.sort((a, b) => b.score - a.score);

  // Build the heavy response object for the top-`limit` only.
  const result = scored.slice(0, limit).map(({ score, p, reasons, clerkHits }) => {
    const dor = String(p.DOR_UC || '').trim();
    const phyAddr = (p.PHY_ADDR1 || '').trim().toUpperCase();
    const ownState = (p.OWN_STATE || '').trim().toUpperCase();
    const jv = parseInt(p.JV) || 0;
    ...
    const sales = [];
    if (sale1Year > 0) sales.push({ price: sale1Price, date: `${p.SALE_MO1 || ''}/${sale1Year}`, year: sale1Year });
    ...
    const cityName = (p.PHY_CITY || cityUp).replace(/_/g, ' ');
    ...
    return {
      ...
      absentee: !!(ownAddr && phyAddr && ownAddr !== phyAddr),
      outOfState: !!(ownState && ownState !== 'FLORIDA' && ownState !== 'FL'),
      corporate: ['LLC', 'CORP', 'INC ', 'INC.', 'HOLDINGS', 'TRUST', 'ESTATE', 'BANK'].some(kw => name.includes(kw)),
      homestead: hmstd > 0,
      bridgePageUrl: `https://title.rootz.global/p/farm?address=${encodeURIComponent(phyAddr)}&city=${encodeURIComponent(cityName)}`
    };
  });
  ...
  return {
    ...
    summary: { high: highN, medium: medN, low: lowN },
