      ...
      if (lines.length > 0) {
        return lines.map(l => {
          try {
            ...
            // Cuyahoga, which publish the SAME data but differ in case (Summit
            // ships lowercase `siteaddress`), qualification (Montgomery ships
            // SDE.WEB_CAMA.*) and column naming. Matching exact-case against
            // Franklin's names returned a record with every field blank,
            // mislabelled as Franklin.
            ...
            const county = fieldIndex.has('PSTLCITY') ? 'Summit'
              : fieldIndex.has('LOC_AREA') ? 'Montgomery'
              : fieldIndex.has('PARCEL_CITY') ? 'Cuyahoga'
              : 'Franklin';
            return {
              // Later alternates are Montgomery's WEB_CAMA columns, which differ
              // from Franklin's by more than case.
              ...
              CONDITION: f('COND') || '',
              GRADE: f('GRADE') || '',
              BASEMENT: f('BASEMENT', 'DWEL_BSMT') || '',
              AC: f('AIRCOND') || '',
              WALL_TYPE: f('WALL', 'DWEL_EXTWALL') || '',
              ROOMS: f('ROOMS', 'DWEL_RMTOT') || 0,
              FIREPLACE: f('FIREPLC', 'DWEL_WBFP_O') || 0,
...
export async function assembleOhioPropertyIntelligence(address, city = '') {
  ...
  // Build source list
  // Attribute to the county the record actually came from. Hardcoding Franklin
  // credited the wrong authority for every Summit/Montgomery/Cuyahoga/Hamilton
  // parcel — a false provenance claim in a block whose purpose is provenance.
  const OH_COUNTY_SOURCE = {
    Franklin: 'Franklin County Auditor (ArcGIS)',
    Hamilton: 'Hamilton County Auditor (CAGIS)',
    Cuyahoga: 'Cuyahoga County Fiscal Officer (ArcGIS)',
    Summit: 'Summit County Fiscal Office (SCGIS)',
    Montgomery: 'Montgomery County Auditor (SDE/WEB_CAMA)',
  };
  // Franklin stays FRANK so previously-issued OH-FRANK-* propertyIds keep resolving.
  const OH_COUNTY_CODE = {
    Franklin: 'FRANK', Hamilton: 'HAMIL', Cuyahoga: 'CUYAH',
    Summit: 'SUMMT', Montgomery: 'MONTG',
  };
  const ohCounty = prop._county || 'Franklin';
  const sourceList = [OH_COUNTY_SOURCE[ohCounty] || `${ohCounty} County Auditor`];
  ...
  return {
    origin: {
      ...
      state: 'OH',
      county: ohCounty,
      propertyId: `OH-${(OH_COUNTY_CODE[ohCounty] || ohCounty.slice(0, 5)).toUpperCase()}-${prop.FOLIO}`,
