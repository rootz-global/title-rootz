...
export function findNearbyRoadWork(lat, lng, maxDistanceMiles = 2.0) {
  ...
  // Coerce before trimming: these come straight from an upstream GIS export
  // whose column types are not guaranteed. CONSTDATE arrived as an epoch number
  // in the road refresh, and `.trim is not a function` took out the whole
  // /api/fl/search endpoint (road work is assembled into every FL property).
  const s = (v) => (v === null || v === undefined ? '' : String(v).trim());
  return roads.slice(0, 20).map(r => ({
    agency: s(r.AGENCY), location: s(r.LOCATION),
    project: s(r.PROJECT), status: s(r.STATUS),
    consultant: s(r.CONSULTANT), constructionDate: s(r.CONSTDATE)
