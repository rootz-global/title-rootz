...
import { FEMA_FLOOD, CENSUS_GEOCODER, CENSUS_ACS, MDC_IDENTIFY, SSL_CERTS,
         CENSUS_API_KEY, CENSUS_KEY_HINT, withCensusKey } from '../lib/constants.js';
...
export async function getCensusData(address, city, state = 'FL', zip = '') {
  ...
  const acsData = await fetchJSON(withCensusKey(acsUrl));
  if (!acsData || acsData.length < 2) {
    // Distinguish "no key configured" from "no data for this geography" —
    // keyless requests redirect to an HTML page and fail opaquely.
    return CENSUS_API_KEY
      ? { error: 'ACS data not available', tract, blockGroup }
      : { error: 'ACS data not available', hint: CENSUS_KEY_HINT, tract, blockGroup };
  }
