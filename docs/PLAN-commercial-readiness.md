...
- [~] **E3 MEASURING** post-E2 recycle rate. BASELINE: **1776 restarts @ epoch 1786586674**. NEXT: after ~20 min read pm2 restarts + `date +%s`, compute Δrestarts/Δhr (was ~8/hr pre-E2). Target <1/hr. If met → mark DONE + lower guard toward 1100–1300M; if improved-but-not-<1/hr → note the residual + move on (E2 already cut it ~73%).
