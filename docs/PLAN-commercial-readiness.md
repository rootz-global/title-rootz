...
- [~] **C2 DEPLOYED, MEASURING** Fixes shipped + healthy (Clark+Lucas pass): (1) census ACS now uses the API key — deploy-drift fix, `<html>` failures went 0 (was constant); (2) fetch cache 5000→1500 entries + timeout 6000→4000ms. BASELINE for recycle-rate check: **1763 restarts @ epoch 1786576971** (right after a manual pm2 restart). NEXT CYCLE: read `pm2 describe title-records` restarts + `date +%s`, compute (Δrestarts / Δhours); target <1/hr (was ~7/hr). If down → mark DONE; else escalate (may need clustering / raise guard given box RAM).
